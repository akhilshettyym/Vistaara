...
const FindSlots = ({
  ...
  const [slotsVisible, setSlotsVisible] = useState(false);
  const [isBooking, setIsBooking] = useState(false);   // <-- new
      ...
      {/* {slotsVisible && (
        <View className="flex-wrap flex-row mx-2 p-2 bg-[#474747] border border-[#1ED760] rounded-lg">
          {slots.map((slot, index) => (
            <TouchableOpacity
              key={index}
              className={`m-2 p-4 border border-[#1ED760] rounded-lg items-center justify-center ${
                selectedSlot && selectedSlot !== slot ? 'opacity-30' : ''
              }`}
