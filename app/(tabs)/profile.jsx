...
const profile = () => {
  ...
  return (
    <View className="flex-1 justify-center items-center bg-[#2b2b2b]">
      <Text className="text-xl text-[#1ED760] font-semibold mb-4">User Profile</Text>
      {userEmail && !isGuest ? (
        <>
          <Text className="text-white text-lg mb-6">Email: {userEmail}</Text>
          <TouchableOpacity onPress={handleLogout} className="p-2 my-2 bg-[#1ED760]  text-black rounded-lg mt-10">
            <Text className="text-lg font-semibold text-center">Logout</Text>
          </TouchableOpacity>
        </>
      ) : (
        <>
          <TouchableOpacity onPress={handleSignup} className="p-2 my-2 bg-[#1ED760]  text-black rounded-lg mt-10">
